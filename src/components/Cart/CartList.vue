...
export default {
  ...
  methods: {
    ...
    async fetchCartProductDetails() {
      try {
        const response = await fetch(
          "https://localhost:7176/api/Cart/GetAllCartProducts"
        );
        if (!response.ok) {
          throw new Error(
    ...
    async deleteCartProductOnServer(cId) {
      const url = `https://localhost:7176/api/Cart/DeleteCartProducts?cId=${cId}`;
    ...
    async saveQuantity(cartProduct) {
      try {
        ...
        const response = await fetch(
          `https://localhost:7176/api/Cart/UpdateCartProducts`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
