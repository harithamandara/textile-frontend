  ...
  methods: {
    ...
    async addToCart(product) {
      try {
        ...
        const cartResponse = await fetch("https://localhost:7176/api/Cart/GetAllCartProducts");
        if (!cartResponse.ok) {
        ...
        const response = await fetch("https://localhost:7176/api/Cart/PostCartProducts", {
          method: "POST",
          headers: {
